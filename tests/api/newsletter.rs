...
async fn newsletter_creation_is_idempotent() {
    ...
    // Mock verfies on Drop that we have sent the newsletter email **once**
}
...
async fn newsletters_are_delivered_to_confirmed_subscribers() {
    ...
    app.dispatch_all_pending_emails().await;
    // Mock verifies on Drop that have sent the newsletter email
}
...
async fn create_unconfirmed_subscriber(app: &TestApp) -> ConfirmationLinks {
    // We are now working with multiple subscribers now,
...
async fn concurrent_form_submission_is_handled_gracefully() {
    ...
    Mock::given(path("/email"))
        ...
        // arrives before the forst one completes
